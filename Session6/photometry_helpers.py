...
def aperture_photometry(data, header, sources, aperture_radius,
                        sky_inner_radius, sky_outer_radius):
    """
    ...
    apertures = p.aperture.CircularAperture(positions, r=aperture_radius)
    sky_annulus = p.aperture.CircularAnnulus(positions,
                                    r_in=sky_inner_radius,
                                    r_out=sky_outer_radius)
    ...
    phot_table[
        'aper_sum_bksub'] = phot_table['aperture_sum'] - phot_table['aper_bkg']
    ...
    with warnings.catch_warnings():
        ...
        wcs = WCS(header)
    ra, dec = wcs.all_pix2world(phot_table['xcenter'], phot_table['ycenter'],
                                0)
    phot_table['RA'] = ra
    phot_table['DEC'] = dec
    ...
    with warnings.catch_warnings():
        warnings.simplefilter('ignore')
        phot_table['instrumental_mag'] = -2.5 * np.log10(
            phot_table['aper_sum_bksub'] / header['EXPOSURE'])
    phot_table['e_instrumental_mag'] = phot_table[
        'aperture_sum_err'] / phot_table['aper_sum_bksub']
...
def plot_sources(data, sources, radius):
    ...
    """
    positions = np.transpose((sources['xcentroid'], sources['ycentroid']))
    apertures = p.aperture.CircularAperture(positions, r=radius)
...
def measure_FWHM(data, sources):
    ...
    mask = reduce(np.logical_and,
                  (sources['flux'] > lims[0], sources['flux']
                   < lims[1], sources['xcentroid'] > 15, sources['ycentroid']
                   > 15, sources['xcentroid'] < data.shape[1] - 15,
    ...
    norm = ImageNormalize(cutout.data,
                          interval=AsymmetricPercentileInterval(1, 99))
    axis[0].imshow(cutout.data,
                   cmap='Greys',
                   origin='lower',
                   norm=norm,
                   interpolation='nearest')
